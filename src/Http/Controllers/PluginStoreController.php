...
use Dcat\Admin\PluginStore\Actions\Plugininstall;
...
use Dcat\Admin\PluginStore\Util\PluginStoreUtil;
    ...
    protected function grid() {
        ...
        $grid = Grid::make(new MarketplaceRepository(), function (Grid $grid) {
            ...
            $grid->actions(function (Grid\Displayers\Actions $actions) {
                ...
                $StoreUser_token = cache()->get('StoreUser_token_'.Admin::user()->id);
                if (!empty($StoreUser_token)) {
                    $paem = [
                        'id'           => $actions->row->id,
                        'product_name' => $actions->row->product_name,
                        'package_name' => $actions->row->package_name,
                        'version'      => $actions->row->last_version,
                    ];
                    $actions->prepend(Plugininstall::make($title)->modalId($actions->row->product_slug)->setKey(json_encode($paem, JSON_UNESCAPED_UNICODE)));
                    $actions->prepend(PluginStoreUtil::installOtherVersion($actions));
                } else {
                    $actions->prepend(PluginStoreUtil::login($title));
                    $actions->prepend(PluginStoreUtil::login('<span class="tips" data-title="安装其它版本"> <i class="fa fa-sort-desc" style="font-size: 20px"></i> &nbsp;&nbsp;</span>') );
                }
            ...
            $grid->tools(PluginStoreUtil::groupBtn($request->url(),$price_full_active,$price_free_active,$price_piad_active));
            $StoreUser_info = cache()->get('StoreUser_info_'.Admin::user()->id);
            if (!empty($StoreUser_info)) {
                $grid->rightTools(PluginStoreUtil::loginToolsBar());
            } else {
                $grid->rightTools(PluginStoreUtil::login('<button class="btn btn-white text-danger"><i class="feather icon-user"></i> 未登陆&nbsp;&nbsp; </button>'));
            }
